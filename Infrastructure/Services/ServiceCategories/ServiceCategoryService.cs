...
namespace Infrastructure.Services.ServiceCategories
{
    public class ServiceCategoryService : IServiceCategoryService
    {
        ...
        public async Task<OperationResult<List<ServiceCategoryDto>>> GetAllAsync(
        ...
        )
        {
            ...
            return OperationResult<List<ServiceCategoryDto>>.Success(
                data,
                "Service categories retrieved successfully"
            );
        }

        public async Task<OperationResult<ServiceCategoryDto>> GetByIdAsync(
            Guid id,
            CancellationToken cancellationToken = default
        )
        {
            ...
            return OperationResult<ServiceCategoryDto>.Success(
                _mapper.Map<ServiceCategoryDto>(category),
                "Service category retrieved successfully"
            );
        }

        public async Task<OperationResult<ServiceCategoryPriceDto>> GetPriceAsync(
            Guid id,
            CancellationToken cancellationToken = default
        )
        {
            ...
            var (minPrice, maxPrice) = await _workerServiceRepository.GetPriceRangeAsync(
                id,
                cancellationToken
            );
        ...
        public async Task<OperationResult<ServiceCategoryDto>> CreateAsync(
        ...
        )
        {
            ...
            if (!parentCheck.IsSuccess)
            {
                return OperationResult<ServiceCategoryDto>.Failure(
                    parentCheck.Message ?? string.Empty
                );
            }
            ...
            _logger.LogInformation(
                "Service category created. Id: {ServiceCategoryId}",
                category.Id
            );
            return OperationResult<ServiceCategoryDto>.Success(
                _mapper.Map<ServiceCategoryDto>(category),
        ...
        public async Task<OperationResult<ServiceCategoryDto>> UpdateAsync(
            Guid id,
        ...
        )
        {
            ...
            if (dto.ImageFile != null)
            {
                category.ImageUrl = await _blobService.UploadImageAsync(
                    dto.ImageFile,
                    category.ImageUrl
                );
            }
            ...
            _logger.LogInformation(
                "Service category updated. Id: {ServiceCategoryId}",
                category.Id
            );
            return OperationResult<ServiceCategoryDto>.Success(
                _mapper.Map<ServiceCategoryDto>(category),
        ...
        )
        {
            ...
            var hasChildren = await _serviceCategoryRepository.HasChildrenAsync(
                id,
                cancellationToken
            );
            ...
            {
                _logger.LogWarning(
                    "Cannot delete service category with children. Id: {ServiceCategoryId}",
                    id
                );
        ...
        private async Task<OperationResult> ValidateParentAsync(
            Guid? parentId,
            Guid? currentId,
        ...
        )
        {
            ...
            var parentExists = await _serviceCategoryRepository.ExistsByIdAsync(
                parentId.Value,
                cancellationToken
            );
        ...
        private async Task<string> GenerateHierarchyCodeAsync(
            Guid? parentId,
            CancellationToken cancellationToken
        )
        {
            var lastCode = await _serviceCategoryRepository.GetLastSiblingCodeAsync(
                parentId,
                cancellationToken
            );
            ...
            if (string.IsNullOrEmpty(lastCode))
            {
                ...
                var parent = await _serviceCategoryRepository.FirstOrDefaultAsync(
