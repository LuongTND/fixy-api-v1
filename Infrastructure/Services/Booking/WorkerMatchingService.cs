...
namespace Infrastructure.Services.Booking
{
    public class WorkerMatchingService : IWorkerMatchingService
    {
            ...
            ILogger<WorkerMatchingService> logger)
        {
            ...
            _workerLocationService = workerLocationService ?? throw new ArgumentNullException(nameof(workerLocationService));
        ...
        /// <inheritdoc />
        public async Task<OperationResult> ProcessAutoMatchAsync(Guid bookingId, CancellationToken cancellationToken = default)
        {
            ...
            {
                _logger.LogWarning("No workers registered for category {CategoryId}", booking.CategoryId);
                return OperationResult.Failure("No workers available for this service category");
            }

            var eligibleWorkerProfileIds = eligibleWorkers.Select(ws => ws.WorkerProfileId).Distinct().ToList();
            ...
            var activeWorkers = await _workerProfileRepository.FindAsync(
                wp => eligibleWorkerProfileIds.Contains(wp.Id)
            ...
            {
                _logger.LogWarning("No active workers available for booking {BookingId}", bookingId);
            ...
            foreach (var worker in activeWorkers)
            {
                ...
                var lastLocation = await _workerLocationService.GetLastLocationAsync(worker.Id, cancellationToken);
                ...
                var distanceKm = CalculateHaversineDistance(
                    ...
                    workerLat.Value, workerLng.Value
                );
            ...
            {
                queueEntries.Add(new WorkerMatchingQueue
                {
                    BookingId = bookingId,
                    WorkerId = worker.Id,
                    AttemptNo = 1,
        ...
        /// <inheritdoc />
        public async Task<OperationResult> OfferToNextWorkerAsync(Guid bookingId, CancellationToken cancellationToken = default)
        {
            var booking = await _bookingRepository.GetByIdAsync(bookingId, cancellationToken);
            if (booking == null)
            {
            ...
            var nextCandidate = await _matchingQueueRepository.GetNextCandidateAsync(bookingId, cancellationToken);
            ...
            if (nextCandidate != null)
            {
                ...
                nextCandidate.ExpiresAt = DateTime.UtcNow.AddMinutes(_matchingSettings.OfferTimeoutMinutes);
                _matchingQueueRepository.Update(nextCandidate);

                // Update booking → assign this worker, set status to Pending
                booking.WorkerId = nextCandidate.WorkerId;
                ...
                _logger.LogInformation(
                    ...
                    bookingId, nextCandidate.WorkerId, nextCandidate.Score, nextCandidate.DistanceKm);
            ...
            else
            {
                ...
                _logger.LogWarning("Booking {BookingId}: No more candidates available. Status set to Matching.", bookingId);
                ...
                await _bookingHubService.SendStatusUpdateAsync(
                    ...
                    new BookingStatusUpdateDto
                    {
                        ...
                        UpdatedAt = DateTime.UtcNow,
                        Message = "No workers available at this time. The system is still searching."
                    },
                    cancellationToken
                );
